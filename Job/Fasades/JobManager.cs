...
namespace JobSpace.Fasades
{
    public sealed class JobManager : IJobManager
    {
        ...
        public void UpdateJob(IJob job, bool getEvent = false)
        {
            try
            {
                Job _job = (Job)job.GetJob();
                _profile.Base.Update(CollectionString, _job);
                if (getEvent)
                {
                    _profile.Plugins.MqController.PublishChanges(MessageEnum.JobChanged, job.Id);
                    OnJobChange(this, _job);
                }
            }
            catch (Exception e)
            {
        ...
        public void CreateManyJobsWithDialog()
        {
            using (var form = new FormAddWorkMany(_profile))
            {
                if (form.ShowDialog() == DialogResult.OK)
                {
                    var jobs = form.CreatedJobs;
